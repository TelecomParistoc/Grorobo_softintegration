...
std::map<int, std::string> Sensor_Thread::sensor_names = {{BLACK_SENSORS_SIDE, "BLACK_SENSORS_SIDE"},
														  {BLACK_SENSOR_MIDDLE, "BLACK_SENSOR_MIDDLE"},
														  {YELLOW_SENSORS_SIDE, "YELLOW_SENSORS_SIDE"},
														  {YELLOW_SENSOR_MIDDLE, "YELLOW_SENSOR_MIDDLE"}};

std::map<int, int> Sensor_Thread::sensor_descriptions_to_ids = {{BLACK_SENSORS_SIDE, 0},
														  		{BLACK_SENSOR_MIDDLE, 1},
														  		{YELLOW_SENSORS_SIDE, 2},
														  		{YELLOW_SENSOR_MIDDLE, 3}};

Sensor_Thread::Sensor_Thread(const std::function<void(int)>& obstacle_callback, const std::function<void(int, bool)>& sensor_callback, bool init) :
...
void Sensor_Thread::init()
{
	...
	_pin_id[sensor_descriptions_to_ids[BLACK_SENSORS_SIDE]] = BLACK_SENSORS_SIDE;
	_pin_id[sensor_descriptions_to_ids[BLACK_SENSOR_MIDDLE]] = BLACK_SENSOR_MIDDLE;
	_pin_id[sensor_descriptions_to_ids[YELLOW_SENSORS_SIDE]] = YELLOW_SENSORS_SIDE;
	_pin_id[sensor_descriptions_to_ids[YELLOW_SENSOR_MIDDLE]] = YELLOW_SENSOR_MIDDLE;

	_pull_up[sensor_descriptions_to_ids[BLACK_SENSORS_SIDE]] = false;
	_pull_up[sensor_descriptions_to_ids[BLACK_SENSOR_MIDDLE]] = false;
	_pull_up[sensor_descriptions_to_ids[YELLOW_SENSORS_SIDE]] = true;
	_pull_up[sensor_descriptions_to_ids[YELLOW_SENSOR_MIDDLE]] = true;

	for(int i=0; i<N_Sensors; i++)
	{
		pinMode(_pin_id[i], INPUT);
		_obstacle_sensors_activated[i] = false;
		if(_pull_up[i])
		{
...
{
	_stop = true;
	if(_acquire_and_react.isjoinable())
...
{
	assert(sensor_descriptions_to_ids.count(desc));
	return _state.at(sensor_descriptions_to_ids.ar(desc));
}

const Sensor_Thread::std::vector<bool>& get_active_sensors()
{
	_update_mutex.lock();
	...
	while(!_stop)
	{
		...
		for(int i=0; i<N_Sensors; i++)
		{
			if(DigitalRead(_pin_id[i]))
			 	if(_pull_up[i] && _cur_scale_pin[i]+1<To_Keep_For_Majority)
					_cur_scale_pin[i]++;
				else if(!_pull_up[i] && _cur_scale_pin[i]>0)
					_cur_scale_pin[i]--;
			else
				if(pull_up[i] && _cur_scale_pin[i]>0)
					_cur_scale_pin[i]--;
				else if(!_pull_up[i] && _cur_scale_pin[i]+1<To_Keep_For_Majority)
					_cur_scale_pin[i]++;
		...
		if(recompute)
		{
			...
			for(int i=0; i<N_Sensors; i++)
				if(_state[i])
					_obstacle_sensors_activated.push_back(_state);
			_update_mutex.unlock();
